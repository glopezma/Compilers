...
(* README
 ...
 ID: P001235175
 Have you ever used a functional programming language before?:
 If so, which one?: Yes, Scheme and lisp
...
(* == EXERCISE 3. == 
   ...
   Without using the multiplication operator [*], define a function
   [triplicate] that takes an integer [x] as argument and returns [3 * x]. *)

let rec trip (x:int) (n:int) : int = 
  match n with
  | 0 -> 0
  | n -> x + trip x (n-1)
;;

let rec triplicate (x:int) : int = 
  trip x 3
;; 
